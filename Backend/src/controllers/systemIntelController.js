...
exports.getSystemIntel = async (req, res, next) => {
    try {
        const intel = await SystemIntel.find().sort({ role: 1 });
        res.status(200).json({
            success: true,
            data: intel
        });
    } catch (error) {
        res.status(500).json({ success: false, message: 'Error fetching system intel', error: error.message });
    }
};
...
exports.updateSystemIntel = async (req, res, next) => {
    try {
        ...
        res.status(500).json({ success: false, message: 'Error updating system intel', error: error.message });
    }
};

// @desc    Seed default system intel
...
exports.seedSystemIntel = async (req, res, next) => {
    try {
        ...
        await SystemIntel.deleteMany(); // Clear existing
        const intel = await SystemIntel.insertMany(defaultData);
        ...
        res.status(500).json({ success: false, message: 'Error seeding system intel', error: error.message });
    }
};
