...
import { ArrowLeft, CheckCircle, Clock, Trash2 } from 'lucide-react';
...
const ManagerProjectDetails = () => {
  ...
  const { projects, fetchProjects, updateProjectStatus, deleteProject } = useProjectStore();
  ...
  const handleDelete = async () => {
    if (window.confirm("CRITICAL: This will permanently purge this project and ALL associated tasks. Proceed with caution. Continue?")) {
      const success = await deleteProject(project._id || project.id);
      if (success) navigate('/manager/projects');
    }
  };
        ...
        <div className="ml-auto flex gap-2">
          {project.status === 'active' && (
            <>
              <Button
                variant="outline"
                onClick={() => updateProjectStatus(project._id || project.id, 'on_hold')}
                className="font-black uppercase tracking-widest text-xs border-amber-200 text-amber-600 bg-amber-50 hover:bg-amber-100"
              >
                Pause Project
              </Button>
              <Button
          ...
          {project.status === 'on_hold' && (
            <Button
              variant="outline"
              onClick={() => updateProjectStatus(project._id || project.id, 'active')}
              className="font-black uppercase tracking-widest text-xs border-indigo-200 text-indigo-600 bg-indigo-50 hover:bg-indigo-100"
            >
              Resume Project
            </Button>
          )}
